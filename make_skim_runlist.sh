...
for file in /cache/hallc/c-lad/raw/*_*.dat.*; do
    # Extract the RUN and NUM parts from the filename
    if [[ $file =~ ^/cache/hallc/c-lad/raw/lad_Production_([0-9]+)\.dat\.([0-9]+)$ ]]; then
        run="${BASH_REMATCH[1]}"
        num="${BASH_REMATCH[2]}"
        #echo $run $num
...
for run in "${!max_num[@]}"; do
    if [ $run -lt $run_max ] && [ $run -gt $run_min ]; then
        ...
        max_file="lad_Production_${run}.dat.${max_num[$run]}"
        size=$(awk -F'=' '$1=="size" { print $2+0 }' "/cache/hallc/c-lad/raw/$max_file")
