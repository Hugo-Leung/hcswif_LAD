...
if not os.path.isdir(lad_raw_dir):
    warnings.warn("raw_dir: " + lad_raw_dir + " does not exist")
...
def parseArgs():
    ...
    parser.add_argument(
        "--events", nargs=1, dest="events", help="number of events to analyze (default=all)"
    )
    parser.add_argument("--name", nargs=1, dest="name", help="workflow name")
    parser.add_argument(
        "--replay",
        nargs=1,
        dest="replay",
    ...
    parser.add_argument("--account", nargs=1, dest="account", help="name of account")
    parser.add_argument("--disk", nargs=1, dest="disk", help="disk space in bytes")
    parser.add_argument("--ram", nargs=1, dest="ram", help="ram space in bytes")
    parser.add_argument("--cpu", nargs=1, dest="cpu", help="cpu cores")
    parser.add_argument(
        "--time",
        nargs=1,
        dest="time",
...
def getReplayJobs(parsed_args, wf_name):
    ...
    if spectrometer.upper() not in [
        ...
        raise ValueError(
            "Spectrometer must be LAD_COIN LAD HMS_ALL, NPS_ALL, VLD_REPLAY, HMS_PROD, NPS_PROD, HMS_COIN, NPS_SKIM, NPS_COIN, NPS_COIN_SCALER, HMS_SCALER, NPS_SCALER"
        )
    ...
    if parsed_args.replay == None:
        ...
        script_dict = {
            "LAD_COIN": "SCRIPTS/LAD_COIN/PRODUCTION/replay_production_lad_spec.C",
            "LAD": "SCRIPTS/LAD/PRODUCTION/replay_production_lad.C",
...
def getCommandJobs(parsed_args, wf_name):
    ...
    for cmd in commands:
        ...
        if parsed_args.filelist == None:
            warnings.warn(
                "No file list specified. Assuming your shell script has any necessary jgets"
            )
        else:
